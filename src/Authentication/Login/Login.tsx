...
import {loginReducer} from '../../redux/ducks/login';
  ...
  const connect = () => {
    if (!state.connecting) {
      if (state.userId && state.nickname) {
        ...
        Keyboard.dismiss();
        sendbird.connect(state.userId, (user: any, err: any) => {
          if (!err) {
    ...
    <>
      ...
      <SafeAreaView style={style.container}>
        <View style={style.logoContainer}>
          <Text style={style.subtitle}>Simple Chat</Text>
          <ActivityIndicator
        ...
        <View style={[style.loginForm]}>
          ...
          <TouchableOpacity
            ...
            style={[
              style.loginButton,
              {backgroundColor: state.connecting ? '#dbcfff' : '#742ddd'},
            ]}
            onPress={connect}>
