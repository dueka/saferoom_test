  ...
  switch (action.type) {
    ...
    case 'fetch-messages': {
      ...
      const distinctMessages = messages.filter(
        (message: any) => !state.messageMap[message.reqId],
      );
    ...
    case 'delete-message': {
      ...
      for (let i in state.messages) {
        if (
          state.messages[i].messageId === messageId ||
          state.messages[i].reqId === reqId
        ) {
          const updatedMessages = state.messages.filter(
            (m: any) => m.reqId !== reqId && m.messageId !== messageId,
          );
          for (let i = 0; i < updatedMessages.length - 1; i++) {
            updatedMessages[i].hasSameSenderAbove =
              ...
              updatedMessages[i].sender.userId ===
                updatedMessages[i + 1].sender.userId;
          }
          return {
            ...state,
            messages: updatedMessages,
