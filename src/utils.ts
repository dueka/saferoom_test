...
export const ellipsis = (s: any, len: any) => {
  return s.length > len ? s.substring(0, len) + '..' : s;
};

export const createChannelName = (channel: any) => {
  if (channel.name === 'Group Channel' || channel.name.length === 0) {
    const nicknames = channel.members.map((m: any) => m.nickname);
...
export const createUnreadMessageCount = (channel: any) => {
...
export const onRemoteMessage = async (remoteMessage: any) => {
    ...
    if (remoteMessage && remoteMessage.data) {
      ...
      if (message && message.channel) {
        ...
        navigation.dispatch((state: any) => {
          ...
          const chatRoute = state.routes.find(
